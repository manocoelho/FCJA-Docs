...
echo Aguardando inicializacao dos modulos...
echo (Isso pode levar alguns segundos a mais na primeira inicializacao)
:: Radar inteligente: checa a porta 8080 em loop. So continua quando a porta responder.
powershell -Command "while($true){try{$tcp=New-Object System.Net.Sockets.TcpClient;$tcp.Connect('127.0.0.1',8080);$tcp.Close();break;}catch{Start-Sleep -Seconds 1}}" > NUL 2>&1
